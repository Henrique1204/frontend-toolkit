...
export const maskDate = (dateText: string = '') => {
  ...
  if (dateTextOnlyNumbers.length > 4) {
    return dateTextOnlyNumbers.replace(/^(\d{2})(\d{2})(\d)/, '$1/$2/$3').substring(0, 10);
  }
...
export const maskPhone = (phoneNumber: string = '') => {
  ...
  if (
    ...
    return (
      ...
      phoneNumberOnlyNumbers.substring(0, AREA_CODE_LENGTH) +
      ') ' +
      phoneNumberOnlyNumbers.substring(AREA_CODE_LENGTH, SHORT_PHONE_NUMBER_LENGTH) +
      '-' +
  ...
  if (phoneNumberOnlyNumbers.length > MEDIUM_PHONE_NUMBER_LENGTH) {
    return (
      ...
      phoneNumberOnlyNumbers.substring(0, AREA_CODE_LENGTH) +
      ') ' +
      phoneNumberOnlyNumbers.substring(AREA_CODE_LENGTH, LONG_PHONE_NUMBER_PART) +
      ' ' +
      phoneNumberOnlyNumbers.substring(LONG_PHONE_NUMBER_PART, EXTENDED_PHONE_NUMBER_PART) +
      '-' +
...
export const maskZipcode = (cep: string = '') => {
  ...
  if (cepOnlyNumbers.length > 5) {
    return cepOnlyNumbers.substring(0, 5) + '-' + cepOnlyNumbers.substring(5, 8);
  }
